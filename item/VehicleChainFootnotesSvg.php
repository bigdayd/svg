...
class VehicleChainFootnotesSvg extends VehicleFootnotesSvg{
	...
	public function __construct(VehicleChainConstructorSvg $chain)
	{
		$this->chain = $chain;
	}
	
	public function addVehicleSizeFootnote(VehicleSvg $vehicle, $size = 'Length', $type = null)
	{
		if (($vn = $this->chain->findVehicle($vehicle))!==null) {
			$sizes = $vehicle->getSizes();
			$offset = $this->chain->getVehicleOffset($vehicle, $type);
			$s = $sizes[$size]; // ['title', 'size', 'orientation'(, 'startOffset', 'real_length')]
			if ($s[2] == VehicleFootnotesSvg::SIZE_FOOTNOTE_HORIZONTAL) {
				$s[3] = (isset($s[3]) ? $s[3] : 0) + $offset[0];
			} else {
				$s[3] = (isset($s[3]) ? $s[3] : 0) + $offset[1];
			}
	...
	public function addSizeFootnote($size = 'Length', $type = null)
	{
		$sizes = $this->chain->getSizes();
		$s = $sizes[$size]; // ['title', 'size', 'orientation'(, 'startOffset', 'real_length')]
		$this->sizeFootnotes[] = $s;
	...
	public function getWidth($type = null)
	{
		...
		foreach ($this->sizeFootnotes as $s) {
			if ($s[2] == self::SIZE_FOOTNOTE_VERTICAL) {
				...
				return $this->chain->getWidth($type);
			}
		}
		return $width;
	}
	...
	public function getHeight($type = null)
	{
		...
		foreach ($this->sizeFootnotes as $s) {
			if ($s[2] == self::SIZE_FOOTNOTE_HORIZONTAL) {
				...
				return $this->chain->getHeight($type);
			}
		}
		return $height;
	}
